...
std::ostream& operator << (std::ostream& os, const Fecha& f1){
    ...
    // aprovechamos la función cadena, para pasar el formato introducido a una cadena.
    os << f1.cadena();
    return os;
}
...
std::istream& operator >> (std::istream& is, Fecha& f1){
    ...
    // Ahora, suponiendo que tenemos bien el formato...
    unsigned i = 0, cont = 0;

    while(format[i] == '0' && cont < 2){
        cont++;
        i++;
    }
    if (cont == 2 && format[i] != '/'){
        is.setstate(std::ios_base::failbit);
        f1 = Fecha();
        throw Fecha::Invalida("Te has pasado de los 2 dígitos del formato.");
    }
    else{
        while(format[i] != '/')
            i++;
